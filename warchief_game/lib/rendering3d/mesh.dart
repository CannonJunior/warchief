...
class Mesh {
  ...
  /// Implementation delegated to _buildMeshTargetIndicator (mesh_factories.dart).
  factory Mesh.targetIndicator({
    double size = 1.5,
    double lineWidth = 0.05,
    Vector3? color,
  }) => _buildMeshTargetIndicator(
    size: size,
    lineWidth: lineWidth,
    color: color ?? Vector3(1.0, 0.2, 0.2),
  );

  /// Create a flat circular disc with radial alpha falloff for aura glow effects.
  /// Implementation delegated to _buildMeshAuraDisc (mesh_factories.dart).
  factory Mesh.auraDisc({
    double radius = 1.0,
    required Vector3 color,
  }) => _buildMeshAuraDisc(radius: radius, color: color);
